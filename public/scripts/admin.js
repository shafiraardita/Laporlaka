...
function handleLogout() {
    ...
    modal.innerHTML = `
        ...
        <div style="margin-top: 15px; display: flex; justify-content: flex-end; gap: 10px;">
            ...
            <button id="logout-yes-btn" style="background-color: #dc3545; color: white;">Keluar</button>
        </div>
    `;
...
function loadProfileData() {
    try {
        ...
        // Simpan ke variabel global juga agar tetap sinkron
...
reports = validateStoredData('reports', reports, validateReportsData);
...
async function fetchLaporanMasuk() {
  try {
    ...
    renderLaporanMasuk(data); // Fungsi untuk menampilkan laporan
  } catch (error) {
...
function renderReportList() {
    try {
        ...
        const dataToRender = filteredReports !== null ? filteredReports : reports.filter(report => report.status === "Masuk");
        ...
        // Hitung indeks awal dan akhir untuk laporan saat ini
        ...
        paginatedReports.forEach(report => {
            ...
            row.innerHTML = `
                ...
                <td>${escapeHTML(report.saksi)}</td>
                <td>${escapeHTML(report.kronologi)}</td>
                <td><span class="report-status ${report.status.toLowerCase()}">${escapeHTML(report.status)}</span></td>
                <td><button onclick="openReportModal(${report.id})"><svg xmlns="http://www.w3.org/2000/svg" width="17" height="17" fill="#375B85" viewBox="0 0 16 16">
        ...
        // Hapus pagination angka jika ada
        const pagination = document.querySelector('#pagination');
        if (pagination) {
            pagination.innerHTML = ''; // Hapus elemen 1, 2, 3
        }
...
function openReportModal(reportId) {
    try {
        ...
        const report = reports.find(r => r.id == reportId); // Gunakan == agar longgar terhadap tipe
        if (!report) {
        ...
        // Logika berdasarkan status laporan
        if (report.status === 'Masuk') {
            petugasInput.disabled = true; // Tidak bisa diisi untuk laporan masuk
            buttonContainer.innerHTML = `
            ...
            petugasInput.disabled = false; // Bisa diisi untuk laporan diterima
            buttonContainer.innerHTML = `
                <button onclick="savePetugas(${reportId})">Simpan</button>
                <button onclick="closeModal()">Batal</button>
            `;
        } else if (report.status === 'Proses') {
            petugasInput.disabled = false; // Bisa diubah untuk penanganan
            buttonContainer.innerHTML = `
                <button onclick="savePetugas(${reportId})">Simpan</button>
                <button onclick="updateStatus(${reportId}, 'Selesai')">Selesai</button>
                <button onclick="closeModal()">Batal</button>
            `;
...
try {
    ...
    console.warn('Error loading accident counts from localStorage:', e);
    accidentCounts2024 = defaultAccidentCounts2024z;
